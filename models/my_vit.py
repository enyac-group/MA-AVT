...
class my_vit(nn.Module):
    ...
    def __init__(self, name='', pretrained=True):
        ...
        self.v = timm.create_model(name, pretrained=pretrained)
    ...
    def forward_patch(self, x, is_shape_info=False, bg_cls_token=None, cnt_token=None):
        ...
        if cnt_token is not None:
            x = torch.cat((
                x,
                cnt_token.expand(x.shape[0], -1, -1)), dim=1)

        if bg_cls_token is not None:
            x = torch.cat((
                x,
                bg_cls_token.expand(x.shape[0], -1, -1)), dim=1)
