...
import cv2
from datasets import AVE, VGGSound, MUSIC, CremadDataset
from models import MBT, LAVISH, Shared_Transformer, Audio_Transformer, Image_Transformer, \
    CLS_Token_Proj_AVIT, CNT_Token_Proj_AVIT, Mean_Pooled_Proj_AVIT, \
        Attn_Pooled_Proj_AVIT, MIM_Proj_AVIT
...
def main(args):
    ...
    args.id += '-MT_{}-UT_{}'.format(args.n_shared_tokens, args.n_audio_tokens)
...
def train(train_loader, model, optimizer, criterion, epoch, history, args):
    ...
    acc_mtr = AverageMeter('Acc', ':6.2f')

    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, loss_mtr, acc_mtr],
    ...
    for i, data in enumerate(train_loader):
        ...
        acc = accuracy(outputs, target, args)

        # measure accuracy and record loss
        loss_mtr.update(loss.item(), image.size(0))
        acc_mtr.update(acc, image.size(0))
        ...
        history['train']['acc'].append(acc_mtr.avg)
...
def validate(test_loader, model, criterion, history, epoch, device, args, prefix='Test'):
    ...
    accs = AverageMeter('Total Acc', ':6.2f')
    ...
    for i, data in enumerate(test_loader):
        ...
        init_time = time.time()
        outputs = model(audio_spec.float(), image.float(), target)
        print("Forward pass time: ", (time.time()-init_time)/audio_spec.shape[0])
    ...
    history['val']['acc'].append(accs.avg)
...
def plot_loss_metrics(path, history, args):
    ...
    plt.plot(history['train']['epoch'], history['train']['acc'],
             color='b', label='Train ACC')
    plt.plot(history['val']['epoch'], history['val']['acc'],
             color='c', label='Val ACC')
